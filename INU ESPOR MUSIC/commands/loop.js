        ...
        col.on('collect', async (button) => {
          ...
          switch (button.customId) {
            case 'queue':
              ...
              interaction?.editReply({ content: `✅ sıra karıştırılıyor!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `✅ karıştırma aktif!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ karıştırma kapalı!!', ephemeral: true }).catch(e => { })
              }
              const success4 = queue.setRepeatMode(0);
              interaction?.editReply({ content: '▶️ kjarıştırma kapalı' }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
          }
        })
